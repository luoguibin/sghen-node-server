...
const JiuGe = {
  ...
  getRandomParams () {
    ...
    return this.getTypeParams(dataUtil.randomItem(types))
  },
  getTypeParams (key = 'jueju') {
    ...
    const params = {
      ...
      level: 1,
      yan: dataUtil.randomItem(jiugeType.yans || this.yans),
      userId: dataUtil.randomString(30),
      isInputValid: jiugeType.isInputValid || function (v) {
        return v && v.length > 0 && v.length < 11
      }
    }
    if (jiugeType.userStyles) {
      params.style = dataUtil.randomIndex(jiugeType.userStyles)
    }
    console.log(params.sghenId, `getTypeParams() ${params}`)
    return params
  }
}
...
const getPoem = function (params, loopCount = 0) {
  console.log(params.sghenId, 'getPoem() start')
  const data = {
  ...
  API.post(`${baseUrl}/getPoem`, data, baseHeaders)
    .then(resp => {
      if (resp.code === '0') {
        ...
        setTimeout(() => {
          console.log(params.sghenId, 'getPoem() continue')
          getPoem(params, loopCount)
        }, 2000)
      } else {
        console.log(params.sghenId, `getPoem() resp:${resp}`)
      }
    }).catch(err => {
      console.log(params.sghenId, `getPoem() err:${err}`)
    })
}

const sendPoem = function (params) {
  console.log(params.sghenId, 'sendPoem() start')
  const data = {
    style: params.style || -1,
    genre: params.genre,
    yan: params.yan,
  ...
  API.post(`${baseUrl}/sendPoem`, data, {
    ...baseHeaders,
    ...
    if (resp.code === '0') {
      ...
      console.log(params.sghenId, `sendPoem() resp:${resp}`)
    }
  }).catch(err => {
    console.log(params.sghenId, `sendPoem() err:${err}`)
  })
}

const sendKeyWord = function (params) {
  console.log(params.sghenId, 'sendKeyWord() start')
  API.post(`${baseUrl}/getKeyword`, {
    level: params.level,
    genre: params.genre,
    ...
    if (resp.code === '0') {
      ...
      console.log(params.sghenId, `sendKeyWord() resp::${resp}`)
    }
  }).catch(err => {
    console.log(params.sghenId, `sendKeyWord() err::${err}`)
  })
}
...
const createPeotry = function (keywords, sghenId) {
  return new Promise(function (resolve, reject) {
    ...
    console.log(originParams.sghenId, 'userCreatePeotry()')
    EventBus.setCall(originParams.userId, data => {
      console.log(originParams.sghenId, 'getPoem() success')
